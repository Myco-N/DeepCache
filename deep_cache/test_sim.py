...
def get_one_hot(probs):
    ...
    for prob in probs:
        ...
        one_hot = torch.FloatTensor(prob.shape).to(device)
        one_hot.zero_()
...
def get_embeddings(addresses, pcs, deepcache):
    ...
    embeddings_pc = deepcache.time_distributed_encoder_mlp(
        pc_embed)  # Convert 4byte embedding to a single address embedding using an MLP
...
def get_freq_rec(probs, dist_vector, deepcache):
    freq_rec = deepcache.get_freq_rec(probs,
                                      dist_vector)  # get freq and rec estimate from prediced probs and distribution vector
    freq = freq_rec[:, 0]
    rec = freq_rec[:, 1]
    return freq, rec
...
def get_freq_rec_ad_pc(dist_vector, deepcache, embeddings):
    ...
    final_embedding = torch.cat([final_embedding, dist_vector],
                                dim=-1)  # concatenate address embedding with dist vector
...
def get_prefetch(misses_address, misses_pc, deepcache):
    hidden_cell = (torch.zeros(1, 1, deepcache.hidden_size).to(device),  # reinitialise hidden state for each new sample
                   torch.zeros(1, 1, deepcache.hidden_size).to(device))
        ...
        for i in tqdm(range(len(addresses))):
            ...
            if address in list(cache_stats.keys()):
                ...
                continue
                    ...
                    cache_stats.keys())) < cache_size:  # If address is not in cache and the cache is not full yet then increment the num_miss
                    ...
                    for pref in prefetch_addresses:
                        if address in list(cache_stats.keys()):
                            ...
                            if int(freq.item() * 10000) > 3000 or int(rec.item() * 10000 + 1) < 7000:
                                ...
                                is_miss, evicted, up_cache = lecar.run(list(cache_stats.keys()), cach_freqs, cach_reqs,
                                                                       pref)
