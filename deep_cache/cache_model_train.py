...
class DeepCache(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.time_distributed_encoder_mlp = TimeDistributed(self.encoder_mlp,
                                                            batch_first=True)  # wrapper function to make encoder time distributed
    ...
    def get_distribution_vector(self, input):
        dist_vector = torch.zeros(input.shape[0], input.shape[2]).to(device)  # initilise the dist vector

        for i in range(input.shape[0]):
            kde = KernelDensity()  # fit KDE
            try:
                kde.fit(input[i].detach().cpu())
            except:
                print('i:', i)
                print('-', *40)
                exit()
    ...
    def forward(self, input, hidden_cell):
        ...
        embeddings_pc = self.time_distributed_encoder_mlp(
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        for (seq, labels) in tqdm(dataloader):
            ...
            optimizer.zero_grad()
            hidden_cell = (
            torch.zeros(1, batch_size, model.hidden_size).to(device),  # reinitialise hidden state for each new sample
