    ...
    def forward(self, inputs, token):
        ...
        for i in range(4):
            ...
            address_inputs.append(
                torch.tensor([token.address_ixs[i][ad[8 * i:8 * (i + 1)]] for ad in inputs[1]], dtype=torch.long))
        ...
        for i in range(4):
            # convert each byte to its index (input to the embedding)
            pc_inputs.append(
                torch.tensor([token.pc_ixs[i][pc[8 * i:8 * (i + 1)]] for pc in inputs[0]], dtype=torch.long))
...
def train(trainer, inputs, tkn, arguments, num):
    ...
    for epoch in range(args.epochs):
        ...
        for trigram in trigrams:
            ...
            inputs = (
            trigram[0][0][0], trigram[1][0][0])  # input to the model are 2 consecutive values for each pc and address
            ...
            for i in range(4):
                targets.append(
                    torch.tensor([token.address_ixs[i][address_trigram[1][8 * i:8 * (i + 1)]]], dtype=torch.long))
...
def get_address(index, model, outputs):
    address = ''
    for i in range(4):
        address += list(model.token.address_ixs[i].keys()[
                            list(model.token.address_ixs[i].values()).index(torch.argmax(outputs[i + 4]))])
    return address
...
def main(args):
    ...
    encoder = ByteEncoder(vocab_sizes_address=vocab_sizes_address, vocab_sizes_pc=vocab_sizes_pc,
