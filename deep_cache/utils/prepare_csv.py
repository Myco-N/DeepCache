...
"""
    ...
    for i in range(len(arr)):
        ...
        if arr[i] not in indexes.keys():
            mp[arr[i]] = [int(len(arr)),
                          i]  # if we see the element for the first time initialize the list as -inf and current index
...
"""
    ...
    for address in arr:
        if len(indexes[address]) >= 1:  # check if the element is repeated
            freq = len(indexes[
                           address]) - 1  # freq is the number of entries in the indexes list except the current element (which is always the 1st one)
